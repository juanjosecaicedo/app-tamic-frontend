  ...
  <div class="wrapper">
    <div class="animated fadeIn">
            ...
            <b-table responsive :hover="true" :striped="true" :bordered="true" :small="true" :items="items"
              ...
              <template slot="actions" slot-scope="data">
                <b-button-group size="sm">
                  <router-link title="Episodios" class="btn btn-success btn-sm" :to="{ name: 'Chapter', params: { 
                      streaming_id: data.item.streaming_id,                       
                      }}">
                  ...
                  <b-button v-b-tooltip.hover title="Eliminar" variant="danger" size="sm"
                    @click="fnt_delete_series(data.item.streaming_id)">
                    <i class="fa fa-trash" />
                  </b-button>
                </b-button-group>
              </template>
      ...
      <b-modal scrollable size="lg" :id="modal.id" :title="modal.title" @hide="fnt_clear_modal">
        <form ref="form" @submit.stop.prevent="fnt_update_series">
          <b-row>
            <b-col sm="4">
          ...
          <b-row>
            <b-col sm="6">
              <b-form-group label="Trailer">
                <b-form-input v-model="url_trailer" required></b-form-input>
              </b-form-group>
            </b-col>
            <b-col sm="6">
        ...
        <template v-slot:modal-footer>
          <b-button variant="primary" class="float-right" @click="fnt_update_series">Guardar</b-button>
        </template>
      </b-modal>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      fnt_update_series() {
        ...
        data.append("tbl_types_type_id", "25caf9e6-7023-442f-b2fc-f33230b219e3");
        data.append("categories", this.select_categories);
        if (!this.flag) {
          this.fnt_add_tags(false);
          axios
            .put("series/" + this.streaming_id, data)
            .then(res => {
              this.fnt_get_series();
          ...
          this.fnt_add_series(data);
        }
      },
      fnt_delete_series(id) {
        ...
        axios
          ...
          .then(res => {
            if (res.data.includes("Ingresar") || res.data.includes("error")) {
              ...
              this.fnt_clear_form();
              fnt_toast("Serie registrada", this.$bvToast);
            }
          })
          .catch(err => {
            fnt_toast(err, this.$bvToast);

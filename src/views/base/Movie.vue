  ...
  <div class="wrapper">
    <div class="animated fadeIn">
              ...
              <template slot="actions" slot-scope="data">
                <b-button-group size="sm">
                  ...
                  <b-button v-b-tooltip.hover title="Eliminar" variant="danger" size="sm"
                    @click="fnt_delete_movies(data.item.streaming_id)">
                    <i class="fa fa-trash" />
                  </b-button>
                </b-button-group>
              </template>
      ...
      <b-modal scrollable size="lg" :id="modal.id" :title="modal.title" @hide="fnt_clear_modal">
        <form ref="form" @submit.stop.prevent="fnt_update_movies">
          <b-row>
            <b-col sm="4">
        ...
        <template v-slot:modal-footer>
          <b-button variant="primary" class="float-right" @click="fnt_update_movies">Guardar</b-button>
        </template>
      </b-modal>
    </div>
  </div>
  ...
  export default {    
    ...
    methods: {
      ...
      fnt_update_movies() {
        ...
        data.append("tbl_types_type_id", "3850baec-fdae-4c39-bf30-33e82a363755");
        data.append("categories", this.select_categories);
        if (!this.flag) {
          this.fnt_add_tags(false);
          axios
            .put("movies/" + this.streaming_id, data)
            .then(res => {
              this.fnt_get_movies();
          ...
          this.fnt_add_movies(data);
        }
      },
      fnt_delete_movies(id) {
        axios
          .delete("streaming/category/" + id)
          .then(() => {
            axios
              .delete("movies/" + id)
              .then(res => {
        ...
        axios
          ...
          .then(res => {
            if (res.data.includes("Ingresar") || res.data.includes("error")) {
              ...
              this.fnt_clear_form();
              fnt_toast("Película registrada", this.$bvToast);
            }
          })
          .catch(err => {
            fnt_toast(err, this.$bvToast);
